...
```{r}
library(mgcv)
df1 <- read.csv("final_functions.csv")
# fit a smooth which varies by comparison_type
df1$type <- factor(df1$type)
b <- gam(dist_search ~ s(phase, k = 4, fx = TRUE, by = type) + type, data = df1, family = betar(link="logit"), method = "REML")
summary(b)
visreg(b, xvar = "phase", by = "type", data = df1, method = "REML")
```

```{r}
df <- read.csv("final_visual.csv")
df2 <- subset(df, df$type != "charts")  
# fit a smooth which varies by comparison_type
df2$type <- factor(df2$type)
b2 <- gam(dist_search ~ s(phase, k = 4, fx = TRUE, by = type) + type, data = df2, family = betar(link="logit"), method = "REML")
summary(b2)
visreg(b2, xvar = "phase", by = "type", data = df2, method = "REML")
```
...
b1 <- gam(dist_search ~ s(phase, k = 4, fx = TRUE, by = type) + type, data = df, family = betar(link="logit"), method = "REML")
summary(b1)
visreg(b1, xvar = "phase", by = "type", data = df, method = "REML")
```

```{r}
library('itsadug')
gamtabs(b1, caption="Complete Visual, Maps and Simple Visual")
```
...
models <- list(
"Vis Code" = gam(dist_search ~ s(phase, k = 4, fx = TRUE, by = type) + type, data = df1, family = betar(link="logit"), method = "REML"),
"Without Chart" = gam(dist_search ~ s(phase, k = 4, fx = TRUE, by = type) + type, data = df2, family = betar(link="logit"), method = "REML"),
"With Chart" = gam(dist_search ~ s(phase, k = 4, fx = TRUE, by = type) + type, data = df, family = betar(link="logit"), method = "REML")
)
